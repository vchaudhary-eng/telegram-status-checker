...
function addRow(obj, idx){
  ...
  tr.innerHTML = `
    ...
    <td>${obj.channel_url !== 'N/A' ? `<a href="${obj.channel_url}" target="_blank">open</a>` : 'N/A'}</td>
    <td>${escapeHtml(obj.channel_name)}</td>
    <td>${obj.subscribers}</td>
    <td>${obj.status}${obj.error ? ' - ' + escapeHtml(obj.error) : ''}</td>
  `;
